...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print("\nExpansions per node:")
            for n, successors in expansions:
                print(f"The node {n} expands {len(successors)} nodes: {successors}")
            return node
        if node.state not in closed:
            closed[node.state] = True
            successors = node.expand(problem)
            fringe.extend(node.expand(problem))
            expansions.append((node, [f"<Node {s.state}>" for s in successors]))
    return None
...
def breadth_first_graph_search(problem):
    ...
    solution = graph_search(problem, queue)
    end_time = perf_counter()
    queue.print_results(solution, start_time, end_time)
    return solution  # FIFOQueue -> fringe
...
def branch_and_bound_with_subestimation_graph_search(problem):
    queue = PriorityQueueWithSubestimation(problem)
    start_time = perf_counter()
    solution = graph_search(problem, queue)
    end_time = perf_counter()
    queue.print_results(solution, start_time, end_time)
