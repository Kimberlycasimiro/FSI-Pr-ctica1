...
class StackQueue:
    ...
    def __init__(self, problem):
        ...
        self.generated_nodes_with_repetition = 0
        self.generated_nodes_without_repetition = set()
        self.visited_nodes_with_repetition = 0
        self.visited_nodes_without_repetition = set()

    def append(self, item):
        self.stack.append(item)
        self.generated_nodes_with_repetition += 1
        self.generated_nodes_without_repetition.add(item.state)
    ...
    def pop(self):
        e = self.stack.pop()
        self.visited_nodes_with_repetition += 1
        self.visited_nodes_without_repetition.add(e.state)
        return e

    def print_results(self, solution, start_time, end_time):
        route = solution.path() if solution else None
        cost = solution.path_cost if solution else None
        print("\nDepth search:")
        print(f"- Nodes generated with repetition: {self.generated_nodes_with_repetition}")
        print(f"- Nodes generated without repetition: {len(self.generated_nodes_without_repetition)}")
        print(f"- Nodes visited with repetition: {self.visited_nodes_with_repetition}")
        print(f"- Nodes visited without repetition: {len(self.visited_nodes_without_repetition)}")
        print(f"- Solution path found: {route}")
        print(f"- Total solution cost: {cost}")
...
class PriorityQueue(Queue):
    ...
    def append(self, item):
        ...
        self.elements.sort(key=lambda n: n.path_cost)
        self.generated_nodes_with_repetition += 1
        self.generated_nodes_without_repetition.add(item.state)
...
class PriorityQueueWithSubestimation(Queue):
    ...
    def append(self, item):
        self.elements.append(item)
        self.elements.sort(key=lambda n: n.path_cost + self.problem.h(n))
        self.generated_nodes_with_repetition += 1
        self.generated_nodes_without_repetition.add(item.state)
    ...
    def print_results(self, solution, start_time, end_time):
        route = solution.path() if solution else None
        cost = solution.path_cost if solution else None
        print("\nBranch and Bound with subestimation search:")
        print(f"- Nodes generated with repetition: {self.generated_nodes_with_repetition}")
        print(f"- Nodes generated without repetition: {len(self.generated_nodes_without_repetition)}")
        print(f"- Nodes visited with repetition: {self.visited_nodes_with_repetition}")
        print(f"- Nodes visited without repetition: {len(self.visited_nodes_without_repetition)}")
